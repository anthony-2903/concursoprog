...
export default function CrudPage() {
  ...
  const fetchClientes = async () => {
    try {
      const res = await fetch('http://204.236.242.154:3000/api/clientes');
...
const handleAddOrUpdate = async (data: Cliente) => {
  if (data.id !== undefined) {
    // Editar
    await fetch(`http://204.236.242.154:3000/api/clientes/${data.id}`, {
      method: 'PUT',
    ...
    // Agregar
    await fetch('http://204.236.242.154:3000/api/clientes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`http://204.236.242.154:3000/api/clientes/${id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
